...
export const handleRoleRemove = async (
    ...
    return {
      embeds: [
        composeEmbedMessage(null, {
          usage: `To remove a specific configuration in a message\n${SLASH_PREFIX}role reaction remove <message_link> <emoji> <role>\n\nTo clear all configurations in a message\n${SLASH_PREFIX}role reaction remove <message_link>`,
          examples: `${SLASH_PREFIX}role reaction remove 967107573591457832 ✅ @Visitor\n${SLASH_PREFIX}role reaction remove 967107573591457832`,
        }),
      ],
    }
  }
  ...
  if (!res.ok) {
    throw new InternalError({
      ...
      title: "Unsuccessful",
      description: `You haven't set this reaction role yet. To set a new one, run \`\`\`${SLASH_PREFIX}role reaction set <message_link> <emoji> <role>\`\`\`\n You can remove it later using \`${SLASH_PREFIX}role reaction remove\`.`,
    })
  }
