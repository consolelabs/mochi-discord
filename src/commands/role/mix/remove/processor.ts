...
export async function process(msg: OriginalMessage) {
      ...
      messageOptions: {
        embeds: [
          getErrorEmbed({
            ...
            description: `No mix roles found! To set a new one, run ${await getSlashCommand(
              "role mix set"
            )}`,
          }),
        ],
  ...
  const { description, fields } = await list({ data })
  const embed = composeEmbedMessage2(msg as CommandInteraction, {
    ...
    messageOptions: {
      embeds: [
        getSuccessEmbed({
          ...
          description: `You can set the new role by using ${await getSlashCommand(
            "role mix list"
          )}`,
        }),
      ],
