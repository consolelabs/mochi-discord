...
export async function list({ data }: ResponseListGuildMixRoles) {
  if (data?.length === 0) {
    return {
      title: "No mix roles found",
      description: `You haven't set any mix roles yet. To set a new one, run ${await getSlashCommand(
        "role mix set"
  ...
  return {
    ...
    description: `You can run ${getSlashCommand(
      "role mix set"
    )} to set the new combined role.\n\n`,
    fields: fields,
  }
}
