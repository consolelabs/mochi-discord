...
class TwitterStream extends InmemoryStorage {
  ...
  private async watchStream() {
    ...
    operation.attempt(async () => {
      try {
        const stream = twitterAppClient.tweets.searchStream({
  ...
  async upsertRule(params: UpsertRuleParams) {
    try {
      ...
      const rule = await twitterAppClient.tweets.addOrDeleteRules({
        add: [
          {
  ...
  async removeRule(params: RemoveRuleParams) {
    await twitterAppClient.tweets.addOrDeleteRules({
      delete: {
        ids: [params.ruleId],
      },
    })
